...
const supabaseUrl = "https://jsneposfxfuyweogmmrl.supabase.co";
const supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImpzbmVwb3NmeGZ1eXdlb2dtbXJsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTYxMjEwNjYsImV4cCI6MjA3MTY5NzA2Nn0.9sRYixKXBZgwRqPxDPCb_0Nx-9n89-JA-LIpS4WiMxY";

if (!supabaseKey) {
  console.warn('⚠️  SUPABASE_ANON_KEY not found. Using local testing mode.');
  console.warn('⚠️  Database operations will be skipped.');
  
  // Create a mock client for local testing
  module.exports = {
    db: {
      getAllProducts: async () => { throw new Error('No database connection - using fallback menu'); },
      createOrder: async () => { throw new Error('No database connection - order will be processed manually'); },
      findOrCreateCustomer: async () => { throw new Error('No database connection'); },
      updateCustomer: async () => { throw new Error('No database connection'); },
      createOrderItems: async () => { throw new Error('No database connection'); }
    }
  };
  return;
}
